...
interface FormState {
  ...
  export default function Contact() {
    ...
    const InputField: React.FC<{ name: keyof FormState; label: string; type?: string }> = ({ name, label, type = 'text' }) => (
      <div className="relative mb-6 group">
        <input
          ...
          className="w-full bg-transparent border-b border-black text-black focus:outline-none peer pt-6"
          placeholder=" "
        />
        <label
          htmlFor={name}
          className={`absolute left-0 top-6 text-black transition-all duration-300 
                      ...
                      ${formState[name] ? '-top-1 text-xs opacity-70' : 'top-6'}`}
        >
          {label}
        </label>
      </div>
    ...
    return (
      <div className="min-h-screen text-black p-6 flex flex-col justify-center">
        ...
        <form onSubmit={handleSubmit} className="max-w-md mx-auto w-full">
          ...
          <div className="relative mb-6 group">
            <textarea
              ...
              onChange={handleChange}
              className="w-full bg-transparent border-b border-black text-black focus:outline-none resize-none peer pt-6"
              rows={4}
              placeholder=" "
            />
            <label
          ...
          <button
            type="submit"
            className="w-full bg-slate-700 text-slate-200 py-3 rounded-full font-bold mt-6 hover:bg-slate-900 transition-colors"
          >
            ENVIAR
          </button>
        </form>
